...
void CAuxEnv::init_camera_parameters(int type) {
	...
	g_config_common.img_source_camera = 1;							// カメラ画像取込み(カメラ[1] / 画像ファイル[0])
	g_config_common.img_source_fname = L"C:\Work\Image.bmp";		//取込み画像ファイル名(CMN_IMAGE_SOURCE_CAMERA = 0のときの画像)
	g_config_common.img_output_fname = L"C:\Work\ImageSave.bmp";	// 画像保存ファイル名
	g_config_common.img_screen_layout.x0 = 25;						//画像表示レイアウト(原点座標X, 原点座標Y, 横幅サイズ, 高さサイズ)
	g_config_common.img_screen_layout.y0 = 120;
	g_config_common.img_screen_layout.width = 680;
	g_config_common.img_screen_layout.height = 480;
	
	//##### CONFIG_CAMERA g_config_common ########
	g_config_camera.basis.ipaddress = L"172.31.0.30";
	// カメラのROI(領域)の設定
	g_config_camera.basis.roi[(int)ENUM_AXIS::X].offset = g_config_camera.basis.roi[(int)ENUM_AXIS::Y].offset = 0;
	g_config_camera.basis.roi[(int)ENUM_AXIS::X].size = 2048; g_config_camera.basis.roi[(int)ENUM_AXIS::Y].size = 1536;
	g_config_camera.basis.packetsize= pCamera->cnfg.packetsize = 0; 
	g_config_camera.basis.framerate = 25.0;			//カメラのフレームレートの設定 フル画像では3fps　512x368で最大50fps位が上限
	g_config_camera.basis.blacklevel= 0.0;			// カメラの黒レベルの設定
	g_config_camera.basis.gamma		= 1.0;			// カメラのガンマ補正値の設定
	g_config_camera.basis.wb.wb_auto = 1;			// カメラのホワイトバランスゲイン(倍率)の設定
	g_config_camera.basis.wb.wb_ratio_red = 1.0;	// カメラのホワイトバランスゲイン(倍率)の設定
	g_config_camera.basis.wb.wb_ratio_blue = 3.4;	// カメラのホワイトバランスゲイン(倍率)の設定
	g_config_camera.basis.viewangle[0] = 41.9127;	//視野角[deg](X, Y)
	g_config_camera.basis.viewangle[1] = 32.0383;	//視野角[deg](X, Y)

	// 輝度コントロール設定(ゲイン)
	g_config_camera.gain.val = 0.0;					//ゲイン設定[dB](初期値, 最小値, 最大値)
	g_config_camera.gain.val_min = 0.0;				//ゲイン設定[dB](初期値, 最小値, 最大値)
	g_config_camera.gain.val_max = 36.0;			//ゲイン設定[dB](初期値, 最小値, 最大値)
	g_config_camera.gain.auto_control = 0;			//自動ゲイン設定(固定[0], 自動ゲイン変化量[dB](未使用))
	g_config_camera.gain.auto_rate = 0;				//自動ゲイン設定(固定[0], 自動ゲイン変化量[dB](未使用))
	g_config_camera.gain.auto_start_h = 0.0;		//自動ゲイン判定輝度(開始上限(この値より輝度が高い場合、暗くする),開始下限(この値より輝度が低い場合、明るくする))[0 - 255]
	g_config_camera.gain.auto_start_l = 0.0;		//自動ゲイン判定輝度(開始上限(この値より輝度が高い場合、暗くする),開始下限(この値より輝度が高い場合、明るくする))[0 - 255]
	g_config_camera.gain.auto_stop_h = 0.0;			//自動ゲイン判定輝度(停止上限(この値より輝度が低い場合、停止する),停止下限(この値より輝度が高い場合、停止する))  [0 - 255]
	g_config_camera.gain.auto_stop_l = 0.0;			//自動ゲイン判定輝度(停止上限(この値より輝度が低い場合、停止する),停止下限(この値より輝度が高い場合、停止する))  [0 - 255]
	
	// 輝度コントロール設定(露光時間)
	g_config_camera.expstime.val = 5000.0;			//ゲイン設定[dB](初期値, 最小値, 最大値)
	g_config_camera.expstime.val_min = 20.0;		//ゲイン設定[dB](初期値, 最小値, 最大値)
	g_config_camera.expstime.val_max = 33000.0;		//ゲイン設定[dB](初期値, 最小値, 最大値)
	g_config_camera.expstime.auto_control = 1;		//自動ゲイン設定(固定[0], 自動ゲイン変化量[dB](未使用))
	g_config_camera.expstime.auto_rate = 5.0;		//自動ゲイン設定(固定[0], 自動ゲイン変化量[dB](未使用))
	g_config_camera.expstime.auto_start_h = 250.0;	//自動露光判定輝度  (開始上限(この値より輝度が高い場合、暗くする)　　[0 - 255]
	g_config_camera.expstime.auto_start_l = 230.0;	//自動露光判定輝度  (開始下限(この値より輝度が低い場合、明るくする)　[0 - 255]
	g_config_camera.expstime.auto_stop_h  = 245.0;	//自動露光判定輝度  (停止上限(この値より輝度が低い場合、停止する)　　[0 - 255]
	g_config_camera.expstime.auto_stop_l  = 235.0;	//自動露光判定輝度  (停止下限(この値より輝度が高い場合、停止する))  [0 - 255]
	
	//カメラの異常判定設定
	g_config_camera.error.framedrop = 15.0;			//カメラの異常判定設定(フレームレート低下の判定値[fps], 異常判定時間[msec])
	g_config_camera.error.errtime	= 100;			//カメラの異常判定設定(フレームレート低下の判定値[fps], 異常判定時間[msec])
		
	// カメラのゲインの設定(APIへの設定はスレッドで実行される)
	g_config_camera.boot.bootwaittimer = 15000;		//カメラの起動設定(システム起動後のカメラ起動待ち時間[msec])
	g_config_camera.boot.rebootcount = 1;			//システム起動後のカメラ起動失敗時のカメラ再起動回数[count]
	g_config_camera.boot.rebootingtimer= 40000;		//カメラ再起動中を通知する時間[msec])

	//##### CONFIG_IMGPROC g_config_imgproc ########
	g_config_imgproc.roi.valid = 1;		//画像ROI設定               (ROI有効設定(なし[0]/あり[1]),ROI倍率)
	g_config_imgproc.roi.scale = 1,5;	//画像ROI設定               (ROI有効設定(なし[0]/あり[1]),ROI倍率)
	
	g_config_imgproc.mask[0].valid = 0;						//マスク画像選択(両方[0] / 画像1のみ[1] / 画像2のみ[2])
	//画像マスクH[0-179] (下限,上限)  画像マスクS[0-255](下限,上限) 画像1マスクV[0-255](下限,上限)
	g_config_imgproc.mask[0].hsv_l[0] = 0; g_config_imgproc.mask[0].hsv_l[1] = 50; g_config_imgproc.mask[0].hsv_l[2] = 150;
	g_config_imgproc.mask[0].hsv_u[0] = 30; g_config_imgproc.mask[0].hsv_u[1] = 255; g_config_imgproc.mask[0].hsv_u[2] = 255;
	g_config_imgproc.mask[1].hsv_l[0] = 50; g_config_imgproc.mask[1].hsv_l[1] = 50; g_config_imgproc.mask[1].hsv_l[2] = 150;
	g_config_imgproc.mask[1].hsv_u[0] = 90; g_config_imgproc.mask[1].hsv_u[1] = 255; g_config_imgproc.mask[1].hsv_u[2] = 255;

	g_config_imgproc.filter[0].val	= 2;//ゴマ塩ノイズフィルター(フィルター設定(なし[0] / 中央値フィルター[1] / オープニング処理[2]), フィルター値(中央値フィルター[1, 3, 5, ...] / オープニング処理[1, 2, ...]))
	g_config_imgproc.filter[0].type = 2;
	g_config_imgproc.filter[1].val	= 0;//穴埋めノイズフィルター(フィルター設定(なし[0] / クロージング処理[1]), フィルター値[1, 2, ...])
	g_config_imgproc.filter[1].type = 9;
	g_config_imgproc.imgprc = 2;				//ターゲット検出アルゴリズム(最大輪郭面積[0], 最大輪郭長[1])
	g_config_imgproc.swayspdfilter = 0.1;		//振れ速度フィルター時定数
	g_config_imgproc.swayzeroset_time = 20000;	//振れゼロ点設定(計測時間[ms])
