...
void CAuxEnv::init_camera_parameters(int type) {
	...
	gp_cnfg_common->img_source_camera = 0;							// カメラ画像取込み(カメラ[1] / 画像ファイル[0])
	...
	gp_cnfg_camera->basis.roi[(int)ENUM_AXIS::X].size = 2048; gp_cnfg_camera->basis.roi[(int)ENUM_AXIS::Y].size = 1536;
