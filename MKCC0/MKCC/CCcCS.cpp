...
static INT16 job_seq_no_last = 0;
...
int CCcCS::input() {
	...
	//### ジョブ起動入力 ###
	if (p_ote_pnl_ctrl[OTE_PNL_CTRLS::auto_act_dbg]) {
		st_cs_work.cs_ctrl.auto_act_req |= CC_CS_DEVICE_INPUT_OTE_PC;
	}
	else {
		st_cs_work.cs_ctrl.auto_act_req &= ~CC_CS_DEVICE_INPUT_OTE_PC;
	}
	if (p_ote_pnl_ctrl[OTE_PNL_CTRLS::auto_act]) {
...
HRESULT CCcCS::set_ote_data_JC(int crane_id) {
	...
	//## ランプ,ブザー表示指令
	{
		...
		st_ote_work.st_body.st_load_stat[0].l = (float)pEnv_Inf->crane_stat.mhl.p;				//ロープ長
		st_ote_work.st_body.st_load_stat[0].T = (float)pEnv_Inf->Tx;							//周期
...
INT32 CCcCS::get_auto_act_input() {
