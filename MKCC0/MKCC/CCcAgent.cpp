...
int CAgent::output() {
	...
	if (g_aux_sway_status) {
		...
		pAUX_CS_Inf->msg_client.body.tg_distance = pEnv_Inf->crane_stat.mhl.p;
...
HRESULT CAgent::trans_plc_io_read_OHC(int crane_id) {

	return S_OK;
}

HRESULT CAgent::plc_io_write_JC(int crane_id) {
	//### PLC書き込み信号処理
//## PCヘルシーカウント
	{
		pc_healthy++;
...
HRESULT CAgent::plc_io_write_OHC(int crane_id) {
...
HRESULT CAgent::aux_equipment_GC(int crane_id) {

	return S_OK;
}
HRESULT CAgent::aux_equipment_OHC(int crane_id) {

	return S_OK;
}
int CAgent::manage_slbrk() {

	slblk_level_fb = (pAUX_CS_Inf->fb_slbrk.brk_fb_rbsl_pos + 100)/600;//旋回ブレーキ制御シリンダ位置レベル判定値

	//### 旋回ブレーキ制御
	//PLCリモートモードのときAuto有効
	if(pPLC_IO->plc_mode_fb & CODE_PLC_CTRL_PLC_REMOTE){//機上遠隔モード時
	...
	if (pCS_Inf->cs_ctrl.ope_pnl_status == CC_CS_CODE_OPEPNL_ACTIVE) {		//遠隔操作端末有効時
		...
		//機能チェックシーケンス中
			...
			if (st_work.slew_brake_ctrl_mode == AG_MODE_SLBK_CHECK_STANDBY) {		//シーケンス開始時
				slblk_chk_cnt--;
				if (slblk_chk_cnt < 0) {											//一定時間待って実行中に移行
			...
			else if (st_work.slew_brake_ctrl_mode == AG_MODE_SLBK_OPT_CHK_FIN) {//チェックシーケンス完了時
				if (pOteCtrl[OTE_PNL_CTRLS::sl_brk] & 0x000F) {//遠隔操作卓ペダル入力有で解除
					st_work.slew_brake_ctrl_mode = AG_MODE_SLBK_NORMAL;
				}
			}
	...
	//### 旋回ブレーキユニットリセット指令
	if (pAUX_CS_Inf->com_slbrk.pc_com_reset = pOteCtrl[OTE_PNL_CTRLS::sl_brk] & AUX_SLBRK_COM_RESET) {		
		pAUX_CS_Inf->com_slbrk.pc_com_brk_level = 0;
	}

	//### 故障リセットでクリア 旋回ブレーキの重故障が発生しているときは旋回ブレーキユニットリセットを入れる
	if (pOteCtrl[OTE_PNL_CTRLS::fault_reset]) {
		//旋回ブレーキの初期チェックNGのクリアを入れる
		pPolInf->pc_fault_map[FLTS_ID_ERR_SLBRK_CHK_NG] &= ~FLTS_MASK_ERR_SLBRK_CHK_NG;

		//旋回ブレーキのエラー出ているときは、旋回ブレーキユニットのリセットを入れる
