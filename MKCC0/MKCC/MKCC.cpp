...
CSharedMem* pEnv_Obj;
CSharedMem* pCS_Obj;
CSharedMem* pScad_Obj;
CSharedMem* pPol_Obj;
CSharedMem* pAgent_Obj;
CSharedMem* pSim_Obj;
                     ...
                     _In_ int       nCmdShow)
{
    ...
    // 共有メモリオブジェクトのインスタンス化
    pEnv_Obj    = new CSharedMem;
    pCS_Obj     = new CSharedMem;
    pScad_Obj   = new CSharedMem;
    pPol_Obj    = new CSharedMem;
    pAgent_Obj  = new CSharedMem;
...
BOOL InitInstance(HINSTANCE hInstance, int nCmdShow)
{
   ...
   st_work_wnd.hWnd = CreateWindowW(szWindowClass, szTitle, WS_OVERLAPPEDWINDOW,
       MAIN_WND_X,
       MAIN_WND_Y,
  ...
  ///-ini file path設定##################
   ...
   ///##Task6 設定 Sim
   {
       ...
       pobj = new CSim;
       VectCtrlObj.push_back(pobj);
       pobj->inf.index = (int32_t)ENUM_THREAD::SIM;
       /// -イベントオブジェクトクリエイト->リスト登録	
       VectHevent.push_back(pobj->inf.hevents[BC_EVENT_TYPE_TIME] = CreateEvent(NULL, FALSE, FALSE, NULL));//自動リセット,初期値非シグナル
