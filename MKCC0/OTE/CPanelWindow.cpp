...
CMainPanelWindow::CMainPanelWindow(HINSTANCE hInstance, HWND hParent, int _crane_id, int _wnd_code, CPanelBase** _ppPanelBase) {
	...
	const wchar_t CLASS_NAME_JC[]		= L"MainWindowJC";
	...
	case CARNE_ID_HHGQ18:
		...
		wcex.hCursor = LoadCursor(nullptr, IDC_ARROW);
		wcex.hbrBackground = (HBRUSH)(COLOR_WINDOW + 1);
		wcex.lpszMenuName = TEXT("OTE MAIN PANEL HHGQ18");
		wcex.lpszClassName = pClassName = CLASS_NAME_JC;
		wcex.hIconSm = NULL;
		break;
...
LRESULT CALLBACK CMainPanelWindow::WndProcJC(HWND hWnd, UINT msg, WPARAM wp, LPARAM lp) {
	...
	case WM_TIMER: {
		...
		if (is_initial_draw_main) {
			pPanelBase->pmainobjs->str_message->update();
			pPanelBase->pmainobjs->str_crane_txt->update(CUIHelper::get_crane_txt_by_code((pCcIf->st_msg_pc_u_rcv.head.myid.machine_id)));
...
LRESULT CALLBACK CMainPanelWindow::WndProcJC_HHGQ18(HWND hWnd, UINT msg, WPARAM wp, LPARAM lp) {
