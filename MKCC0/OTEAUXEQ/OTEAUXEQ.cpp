// OTEAUXEQ.cpp : アプリケーションのエントリ ポイントを定義します。
//
...
static std::string imName = "USB CAMERA";       //opencv image name for display
                     ...
                     _In_ int       nCmdShow)
{
    ...
    LoadStringW(hInstance, IDC_OTEAUXEQ, szWindowClass, MAX_LOADSTRING);
    ...
    HACCEL hAccelTable = LoadAccelerators(hInstance, MAKEINTRESOURCE(IDC_OTEAUXEQ));
...
ATOM MyRegisterClass(HINSTANCE hInstance)
{
    ...
    wcex.hIcon          = LoadIcon(hInstance, MAKEINTRESOURCE(IDI_OTEAUXEQ));
    wcex.hCursor        = LoadCursor(nullptr, IDC_ARROW);
    wcex.hbrBackground  = (HBRUSH)(COLOR_WINDOW+1);
    wcex.lpszMenuName   = MAKEINTRESOURCEW(IDC_OTEAUXEQ);
...
BOOL InitInstance(HINSTANCE hInstance, int nCmdShow)
{
   ...
   ///-共有メモリ割付&設定##################
   if (OK_SHMEM != pEnvInfObj->create_smem(SMEM_OTE_AUX_ENV_INF_NAME, sizeof(ST_AUX_ENV_INF), MUTEX_OTE_AUX_ENV_INF_NAME)) return(FALSE);
   if (OK_SHMEM != pAgentInfObj->create_smem(SMEM_OTE_AUX_AGENT_INF_NAME, sizeof(ST_AUX_AGENT_INF), MUTEX_OTE_AUX_AGENT_INF_NAME)) return(FALSE);
   if (OK_SHMEM != pCsInfObj->create_smem(SMEM_OTE_AUX_CS_INF_NAME, sizeof(ST_AUX_CS_INF), MUTEX_OTE_AUX_CS_INF_NAME)) return(FALSE);
   if (OK_SHMEM != pPolInfObj->create_smem(SMEM_OTE_AUX_POL_INF_NAME, sizeof(ST_AUX_CS_INF), MUTEX_OTE_AUX_POL_INF_NAME)) return(FALSE);
   ...
   ///##Task2 設定 Client Service
   {
       ...
       ///オブジェクト名セット
       DWORD	str_num = GetPrivateProfileString(OBJ_NAME_SECT_OF_INIFILE, CS_KEY_OF_INIFILE, L"Client Service", taskname, BC_TASK_NAME_BUF_SIZE, PATH_OF_INIFILE);
       pobj->inf.name.clear(); pobj->inf.name.append(taskname);
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    break;
    case WM_USER_OTE_AUX_DISP_CAMERA_CTRL:
    {
    }break;
...
{
    RECT rc;
    TC_ITEM tc[N_AUXEQ_TASK];//タブコントロール設定構造体
