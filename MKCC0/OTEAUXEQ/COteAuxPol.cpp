...
cv::Mat COteAuxPol::mat_mask;
cv::Mat COteAuxPol::rgbMat_mask;
std::mutex COteAuxPol::m_maskMutex;
std::vector<std::vector<cv::Point>> COteAuxPol::m_allContours;   // 輪郭データ
std::mutex COteAuxPol::m_contourMutex;                           // 輪郭データ保護用

INT32 COteAuxPol::pol_video_delay_chk_ctrl = 0;

ST_OTE_AUX_POL_INF COteAuxPol::st_aux_pol_inf;

static LPST_AUXPOL_IMG_PRM pImgPrm = NULL;
static cv::Mat init_work_mat1;
static cv::Mat init_work_mat2;
static cv::Rect init_work_roi;
static std::mutex diffmatMutex;
static cv::Mat diff_mat;
static LONGLONG AROIarea=0;
...
static LPST_OTE_CC_IF			pCCIf = NULL;

static wstring prmfile_type_str = L"OTEAUX_";
...
HRESULT COteAuxPol::initialize(LPVOID lpParam) {
	...
	pst_img_proc->param.hl = 0; pst_img_proc->param.hh = 10;  pst_img_proc->param.hCenter = 0;
	pst_img_proc->param.sl = 0; pst_img_proc->param.sh = 255; pst_img_proc->param.sCenter = 0;
	pst_img_proc->param.vl = 0; pst_img_proc->param.vh = 255; pst_img_proc->param.vCenter = 0;
	...
	pImgPrm->rc_mat_roi_criterion		= { AUXPOL_DEFAULT_ROI_X, AUXPOL_DEFAULT_ROI_Y, AUXPOL_DEFAULT_ROI_W,AUXPOL_DEFAULT_ROI_H };
...
HRESULT COteAuxPol::routine_work(void* pObj) {
	...
	if (inf.total_act % 20 == 0) {
		...
		wos << L" APRM STEP:" << pst_img_proc->v_delay_auto_prm_step;
		msg2host(wos.str());
	}
...
int COteAuxPol::parse() {           //メイン処理
	...
	//### パラメータセーブ要求処理
	if (((pCSInf->video_delay_chk_ctrl & OTE_CS_CODE_V_DELAY_COM_PRM_SAVE)||(pol_video_delay_chk_ctrl & OTE_CS_CODE_V_DELAY_COM_PRM_SAVE)) &&
		!(pst_img_proc->v_delay_prm_io_status & OTEAUXPOL_CODE_V_DELAY_PRM_SAVE_START)) {
		st_aux_pol_inf.st_img_proc.v_delay_prm_io_status |= OTEAUXPOL_CODE_V_DELAY_PRM_SAVE_START;
		st_aux_pol_inf.st_img_proc.v_delay_prm_io_status &= ~(OTEAUXPOL_CODE_V_DELAY_PRM_SAVE_FIN | OTEAUXPOL_CODE_V_DELAY_PRM_SAVE_FAIL);
		//パラメータ保存処理開始
		if (SaveParameters_Vdelay(&(pCCIf->st_msg_pc_u_rcv.head.myid)) == S_OK) {
			st_aux_pol_inf.st_img_proc.v_delay_prm_io_status |= OTEAUXPOL_CODE_V_DELAY_PRM_SAVE_FIN;
		}
		else {
			st_aux_pol_inf.st_img_proc.v_delay_prm_io_status |= OTEAUXPOL_CODE_V_DELAY_PRM_SAVE_FAIL;
		}
		//デバッグ用要求フラグクリア
	...
	//### パラメータロード要求処理
	if (((pCSInf->video_delay_chk_ctrl & OTE_CS_CODE_V_DELAY_COM_PRM_LOAD)||(pol_video_delay_chk_ctrl & OTE_CS_CODE_V_DELAY_COM_PRM_LOAD)) &&
		...
		if (LoadParameters_Vdelay(&(pCCIf->st_msg_pc_u_rcv.head.myid)) == S_OK) {
			...
			pst_img_proc->v_delay_prm_io_status |= OTEAUXPOL_CODE_V_DELAY_PRM_LOAD_FAIL;
		}
		//デバッグ用要求フラグクリア
	...
	if ((pCSInf->video_delay_chk_ctrl & OTE_CS_CODE_V_DELAY_COM_AUTO_PRM)|| (pol_video_delay_chk_ctrl & OTE_CS_CODE_V_DELAY_COM_AUTO_PRM)) {
		auto_calibrate_req = true;
	}
	else{
		auto_calibrate_req = false;
	}
	
	if (auto_calibrate_req && !(st_aux_pol_inf.st_img_proc.v_delay_prm_io_status & OTEAUXPOL_CODE_V_DELAY_AUTO_PRM_START)) {
		st_aux_pol_inf.st_img_proc.v_delay_prm_io_status |= OTEAUXPOL_CODE_V_DELAY_AUTO_PRM_START;
		//自動パラメータ設定処理開始 -> 自動パラメータ設定のステップをOFF画像取得にセット

		st_aux_pol_inf.st_img_proc.v_delay_auto_prm_step = OTEAUXPOL_CODE_V_DELAY_APARAM_STEP_GET_OFF_MAT;
		st_aux_pol_inf.st_img_proc.video_delay_auto_prm_status = L"OFF画像取得中";
	}
	else if (!auto_calibrate_req) {
		//自動パラメータ設定要求がない場合はステップを待機状態に
		st_aux_pol_inf.st_img_proc.v_delay_prm_io_status &= ~OTEAUXPOL_CODE_V_DELAY_AUTO_PRM_START;
		pst_img_proc->v_delay_auto_prm_step = OTEAUXPOL_CODE_V_DELAY_APARAM_STEP_STANDBY;
	}
	else;
	...
	// 2. 画像処理の基準値(pixlがON：検出条件成立の数）が未設定では処理しない
	if (pst_img_proc->param.target_chk_base_count)
	...
	// 3. 検出パラメータ調整
	int tem_int = 0;
	if (pst_img_proc->param.sl > pst_img_proc->param.sh) {//彩度閾値の下限が上限より大きい場合は入れ替える
		tem_int = pst_img_proc->param.sl;
		pst_img_proc->param.sl = pst_img_proc->param.sh;
		pst_img_proc->param.sh = tem_int;
	}
	if (pst_img_proc->param.vl > pst_img_proc->param.vh) {//明度閾値の下限が上限より大きい場合は入れ替える
		tem_int = pst_img_proc->param.vl;
		pst_img_proc->param.vl = pst_img_proc->param.vh;
		pst_img_proc->param.vh = tem_int;
	}

	// 4. 検出処理
	pst_img_proc->is_target_detected = GetCraneDeviceStatus(pst_img_proc);

	return STAT_OK;
}
int COteAuxPol::output() {          //出力処理

	*pAuxPolInf = st_aux_pol_inf;
...
int COteAuxPol::GetCraneDeviceStatus(LPST_AUXPOL_IMG_PROC pst_work) {
	...
	if (pst_work->param.hl > pst_work->param.hh) {//色相閾値の下限が上限より大きい場合は、0-180の両端で範囲を設定してチェック結果をORする	
		cv::inRange(pAuxAgInf->st_usb_cam.hsvMatFrame, cv::Scalar(0, pst_work->param.sl, pst_work->param.vl), cv::Scalar(pst_work->param.hh, pst_work->param.sh, pst_work->param.vh), m1);
		cv::inRange(pAuxAgInf->st_usb_cam.hsvMatFrame, cv::Scalar(pst_work->param.hl, pst_work->param.sl, pst_work->param.vl), cv::Scalar(179, pst_work->param.sh, pst_work->param.vh), m2);
		cv::bitwise_or(m1, m2, local_mask);
	}
	else {
		cv::inRange(pAuxAgInf->st_usb_cam.hsvMatFrame, cv::Scalar(pst_work->param.hl, pst_work->param.sl, pst_work->param.vl), cv::Scalar(pst_work->param.hh, pst_work->param.sh, pst_work->param.vh), local_mask);
	}
	
	{//mutexでmat_maskロック
		std::lock_guard<std::mutex> lock(m_maskMutex);
		mat_mask = local_mask.clone();

	// 3. マスク画像からROI内の白いピクセル数をカウント
		pst_work->whiteCountInCriterionRoi = cv::countNonZero(mat_mask(pImgPrm->rc_mat_roi_criterion));	//設定した基準Roi内の白いピクセル数をカウント
		pst_work->whiteCountInWorkRoi = cv::countNonZero(mat_mask(rc_mat_roi_work));				//設定したRoi内の白いピクセル数をカウント	
	}//ロック解除
...
HRESULT COteAuxPol::CheckVdlayParameters(ST_AUXPOL_IMG_PRM st_param) {
	if(st_param.magic != OTEAUXPOL_CODE_MAGIC) {
		return S_FALSE;
	}
	if( st_param.hl < -1 || st_param.hl > 180 ||
		st_param.hh < -1 || st_param.hh > 180 ||
		st_param.sl < -1 || st_param.sl > 255 ||
		st_param.sh < -1 || st_param.sh > 255 ||
		st_param.vl < -1 || st_param.vl > 255 ||
		st_param.vh < -1 || st_param.vh > 255) {
			return S_FALSE;
	}

	if( st_param.h_margin < 0 || st_param.s_margin < 0 || st_param.v_margin < 0) {
		return S_FALSE;
	}

	if( st_param.rc_mat_roi_criterion.x < 0 || st_param.rc_mat_roi_criterion.y < 0 ||
		st_param.rc_mat_roi_criterion.width <= 0 || st_param.rc_mat_roi_criterion.height <= 0) {
			return S_FALSE;
	}

	if ((st_param.rc_mat_roi_criterion.x + st_param.rc_mat_roi_criterion.width >= AUXPOL_CAMERA_W )||
		(st_param.rc_mat_roi_criterion.y + st_param.rc_mat_roi_criterion.height >= AUXPOL_CAMERA_H)) {
		return S_FALSE;
	}
...
HRESULT COteAuxPol::LoadParameters_Vdelay(LPST_DEVICE_CODE pCrane) {
	...
	ST_AUXPOL_IMG_PRM temp_prm; //一時的な構造体変数を用意
	...
	//識別子が一致する場合のみ反映（ファイルが壊れていないかチェック）
		if (S_OK == CheckVdlayParameters(temp_prm)) {
			// GUIのスライダー位置を更新
...
HRESULT COteAuxPol::AutoParameterts_Vdelay(INT32 step) {
	...
	case OTEAUXPOL_CODE_V_DELAY_APARAM_STEP_GET_ROI: {
		...
		double maxArea = 0;
		int maxIdx = -1;
		for (int i = 0; i < m_allContours.size(); ++i) {
			double area = cv::contourArea(m_allContours[i]);
			if (area > maxArea) {
				maxArea = area;
				maxIdx = i;
			}
		...
		if (maxIdx != -1 && maxArea > AUXPOL_PRM_VDELAY_AUTO_CHK_MIN_AREA) {
			...
			if (AROIarea > 10000) {//あまりに大きな領域は誤検出の可能性があるため無視（例: 100x100=10000px）
				wos.str(L""); wos << L"失敗(領域大:" << AROIarea;
				st_aux_pol_inf.st_img_proc.video_delay_auto_prm_status = wos.str().c_str();
				st_aux_pol_inf.st_img_proc.v_delay_auto_prm_step = OTEAUXPOL_CODE_V_DELAY_APARAM_STEP_FAIL;
			}
			else {
				init_work_roi = boundingBox;
				st_aux_pol_inf.st_img_proc.v_delay_auto_prm_step = OTEAUXPOL_CODE_V_DELAY_APARAM_STEP_SET_PRM;
...
LRESULT CALLBACK COteAuxPol::Mon2Proc(HWND hWnd, UINT msg, WPARAM wp, LPARAM lp) {
	...
	case WM_CREATE: {
		...
		st_mon2.hctrl[AUXPOL_ID_MON2_SLIDER_HL] = CreateSlider(hWnd, AUXPOL_ID_MON2_CTRL_BASE + AUXPOL_ID_MON2_SLIDER_HL, 680, 50, -1, 180, pst_img_proc->param.hl);
		st_mon2.hctrl[AUXPOL_ID_MON2_SLIDER_HH] = CreateSlider(hWnd, AUXPOL_ID_MON2_CTRL_BASE + AUXPOL_ID_MON2_SLIDER_HH, 680, 90, -1, 180, pst_img_proc->param.hh);
		st_mon2.hctrl[AUXPOL_ID_MON2_SLIDER_SL] = CreateSlider(hWnd, AUXPOL_ID_MON2_CTRL_BASE + AUXPOL_ID_MON2_SLIDER_SL, 680, 150, -1, 256, pst_img_proc->param.sl);
		st_mon2.hctrl[AUXPOL_ID_MON2_SLIDER_SH] = CreateSlider(hWnd, AUXPOL_ID_MON2_CTRL_BASE + AUXPOL_ID_MON2_SLIDER_SH, 680, 190, -1, 256, pst_img_proc->param.sh);
		st_mon2.hctrl[AUXPOL_ID_MON2_SLIDER_VL] = CreateSlider(hWnd, AUXPOL_ID_MON2_CTRL_BASE + AUXPOL_ID_MON2_SLIDER_VL, 680, 250, -1, 256, pst_img_proc->param.vl);
		st_mon2.hctrl[AUXPOL_ID_MON2_SLIDER_VH] = CreateSlider(hWnd, AUXPOL_ID_MON2_CTRL_BASE + AUXPOL_ID_MON2_SLIDER_VH, 680, 290, -1, 256, pst_img_proc->param.vh);
	...
	case WM_RBUTTONDOWN: {//右クリックで選択解除
			st_mon2.flg_dispDragging = false;
			st_mon2.rc_selected = cv::Rect(0, 0, 0, 0);
	}break;
	case WM_HSCROLL: {
		int id = GetDlgCtrlID((HWND)lp);
		int pos = (int)SendMessage((HWND)lp, TBM_GETPOS, 0, 0);
		if (id == AUXPOL_ID_MON2_CTRL_BASE + AUXPOL_ID_MON2_SLIDER_HL) {
			pst_img_proc->param.hl = pos;monwos.str(L""); monwos << pst_img_proc->param.hl;
			SetWindowText(st_mon2.hctrl[AUXPOL_ID_MON2_STATIC_HL], monwos.str().c_str());
		}
		if (id == AUXPOL_ID_MON2_CTRL_BASE + AUXPOL_ID_MON2_SLIDER_HH) {
			pst_img_proc->param.hh = pos; monwos.str(L""); monwos << pst_img_proc->param.hh;
			SetWindowText(st_mon2.hctrl[AUXPOL_ID_MON2_STATIC_HH], monwos.str().c_str());
		}
		if (id == AUXPOL_ID_MON2_CTRL_BASE + AUXPOL_ID_MON2_SLIDER_SL) {
			pst_img_proc->param.sl = pos; monwos.str(L""); monwos << pst_img_proc->param.sl;
			SetWindowText(st_mon2.hctrl[AUXPOL_ID_MON2_STATIC_SL], monwos.str().c_str());
		}
		if (id == AUXPOL_ID_MON2_CTRL_BASE + AUXPOL_ID_MON2_SLIDER_SH) {
			pst_img_proc->param.sh = pos; monwos.str(L""); monwos << pst_img_proc->param.sh;
			SetWindowText(st_mon2.hctrl[AUXPOL_ID_MON2_STATIC_SH], monwos.str().c_str());
		}
		if (id == AUXPOL_ID_MON2_CTRL_BASE + AUXPOL_ID_MON2_SLIDER_VL) {
			pst_img_proc->param.vl = pos; monwos.str(L""); monwos << pst_img_proc->param.vl;
			SetWindowText(st_mon2.hctrl[AUXPOL_ID_MON2_STATIC_VL], monwos.str().c_str());
		}
		if (id == AUXPOL_ID_MON2_CTRL_BASE + AUXPOL_ID_MON2_SLIDER_VH) {
			pst_img_proc->param.vh = pos; monwos.str(L""); monwos << pst_img_proc->param.vh;
			SetWindowText(st_mon2.hctrl[AUXPOL_ID_MON2_STATIC_VH], monwos.str().c_str());
		}
		return 0;
	}
		...
		case IDC_TASK_ITEM_CHECK6:
		{
			if (inf.panel_func_id == IDC_TASK_FUNC_RADIO1) {
				...
				if (LOWORD(wp) == IDC_TASK_ITEM_CHECK1) {
					st_mon2.disp_mode = AUXPOL_CODE_MON2_DISP_H;
				}
				else if (LOWORD(wp) == IDC_TASK_ITEM_CHECK2) {
					st_mon2.disp_mode = AUXPOL_CODE_MON2_DISP_S;
				}
				else if (LOWORD(wp) == IDC_TASK_ITEM_CHECK3) {
					st_mon2.disp_mode = AUXPOL_CODE_MON2_DISP_V;
				}
				else if (LOWORD(wp) == IDC_TASK_ITEM_CHECK4) {
					st_mon2.disp_mode = AUXPOL_CODE_MON2_DISP_HSV_MASK;
				}
				else if (LOWORD(wp) == IDC_TASK_ITEM_CHECK5) {
					st_mon2.disp_mode = AUXPOL_CODE_MON2_DISP_APRM_DIFF;
				}
			...
			if (inf.panel_func_id == IDC_TASK_FUNC_RADIO2) {
				...
				if (LOWORD(wp) == IDC_TASK_ITEM_CHECK1) {
					if (pol_video_delay_chk_ctrl & OTE_CS_CODE_V_DELAY_COM_AUTO_PRM)
						pol_video_delay_chk_ctrl &= ~OTE_CS_CODE_V_DELAY_COM_AUTO_PRM;
					else
						pol_video_delay_chk_ctrl |= OTE_CS_CODE_V_DELAY_COM_AUTO_PRM;
					...
					if (pol_video_delay_chk_ctrl & OTE_CS_CODE_V_DELAY_COM_PRM_SAVE)
						pol_video_delay_chk_ctrl &= ~OTE_CS_CODE_V_DELAY_COM_PRM_SAVE;
					else
						pol_video_delay_chk_ctrl |= OTE_CS_CODE_V_DELAY_COM_PRM_SAVE;
					...
					if (pol_video_delay_chk_ctrl & OTE_CS_CODE_V_DELAY_COM_PRM_LOAD)
						pol_video_delay_chk_ctrl &= ~OTE_CS_CODE_V_DELAY_COM_PRM_LOAD;
					else
						pol_video_delay_chk_ctrl |= OTE_CS_CODE_V_DELAY_COM_PRM_LOAD;
...
void COteAuxPol::set_item_chk_txt() {
	if(inf.panel_func_id == IDC_TASK_FUNC_RADIO1) {
		SetDlgItemText(inf.hwnd_opepane, IDC_TASK_ITEM_CHECK1, L"HMat");
		SetDlgItemText(inf.hwnd_opepane, IDC_TASK_ITEM_CHECK2, L"SMat");
		SetDlgItemText(inf.hwnd_opepane, IDC_TASK_ITEM_CHECK3, L"VMat");
		SetDlgItemText(inf.hwnd_opepane, IDC_TASK_ITEM_CHECK4, L"HSVMask");
		SetDlgItemText(inf.hwnd_opepane, IDC_TASK_ITEM_CHECK5, L"APRM DIFF");
...
void COteAuxPol::UpdateMon2(HWND hWnd, HDC hdc) {
	...
	if (!pAuxAgInf->st_usb_cam.bgrMatFrame.empty()) {
		...
		if(st_mon2.flg_disp_cri_roi) {
			monwos.str(L""); h_text += 25.0f;
			monwos << L"(" << pImgPrm->rc_mat_roi_criterion.x << L"," << pImgPrm->rc_mat_roi_criterion.y << L")" << L" (" << pImgPrm->rc_mat_roi_criterion.x + pImgPrm->rc_mat_roi_criterion.width<< L"," << pImgPrm->rc_mat_roi_criterion.y + pImgPrm->rc_mat_roi_criterion.height << L")"
				<< L" HSV:[" << pst_img_proc->param.hCenter<< L"," << pst_img_proc->param.sCenter << L"," << pst_img_proc->param.vCenter << L"]"
				<< L" Cnt:" << pst_img_proc->whiteCountInCriterionRoi << L"(" << pst_img_proc->param.target_chk_base_count << L")";
	...
	if (st_mon2.disp_mode == AUXPOL_CODE_MON2_DISP_HSV_MASK) {
		...
		if (!mat_mask.empty()) {
			{//mutexでmat_maskロック
				std::lock_guard<std::mutex> lock(m_maskMutex);
				cv::cvtColor(mat_mask, rgbMat_mask, cv::COLOR_GRAY2RGB);
			}//ロック解除
			// --- 手順1: Bitmapバッファへ順次描画 ---

			Gdiplus::Bitmap bitmap(rgbMat_mask.cols, rgbMat_mask.rows, (int)rgbMat_mask.step, PixelFormat24bppRGB, rgbMat_mask.data);
			m_pOffscreenGraphics->DrawImage(&bitmap, 0, height, width, height);

			//テキスト描画
			Font font(L"Arial", 12, FontStyleBold);
			monwos.str(L"HSV MASK ●");
			if (pst_img_proc->status & AUXPOL_CODE_IMG_PROC_ACTIVE) {
				if (pst_img_proc->is_target_detected)
					m_pOffscreenGraphics->DrawString(monwos.str().c_str(), -1, &font, PointF(5.0f, 5.0f + height), m_pMagentaBrush.get());
				else
					m_pOffscreenGraphics->DrawString(monwos.str().c_str(), -1, &font, PointF(5.0f, 5.0f + height), m_pBlueBrush.get());
			}
			else {
	...
	else if ((st_mon2.disp_mode == AUXPOL_CODE_MON2_DISP_H)|| (st_mon2.disp_mode == AUXPOL_CODE_MON2_DISP_S)|| (st_mon2.disp_mode == AUXPOL_CODE_MON2_DISP_V)) {
		//HSV個別画像の表示処理
		if (pAuxAgInf->st_usb_cam.hsvMatFrame.empty()) return;
		cv::Mat Vmat,RGBmat;

		{
			std::lock_guard<std::mutex> lock(pAuxAgInf->hsvMutex);// hsvMatFrameへのアクセスをロック
			int channel = st_mon2.disp_mode - 1;
			cv::extractChannel(pAuxAgInf->st_usb_cam.hsvMatFrame, Vmat, channel);
			cv::cvtColor(Vmat, RGBmat, cv::COLOR_GRAY2BGR);

		}//ロック解除

		Gdiplus::Bitmap bitmap(RGBmat.cols, RGBmat.rows, (int)RGBmat.step, PixelFormat24bppRGB, RGBmat.data);

		width = Vmat.cols / AUXPOL_CAMERA_DISP_RETIO;
		height = Vmat.rows / AUXPOL_CAMERA_DISP_RETIO;
		m_pOffscreenGraphics->DrawImage(&bitmap, 0, height, width, height);

		// (任意) 画面に「V-Channel」とテキストを入れる
		Font font(L"Arial", 9, FontStyleBold);
		SolidBrush yellowBrush(Color::Yellow);
		if (st_mon2.disp_mode == AUXPOL_CODE_MON2_DISP_H)
			m_pOffscreenGraphics->DrawString(L"H-Channel (Hue)", -1, &font, PointF(10, height + 10), &yellowBrush);
		else if (st_mon2.disp_mode == AUXPOL_CODE_MON2_DISP_S)
			m_pOffscreenGraphics->DrawString(L"S-Channel (Saturation)", -1, &font, PointF(10, height + 10), &yellowBrush);
		else if (st_mon2.disp_mode == AUXPOL_CODE_MON2_DISP_V)
			m_pOffscreenGraphics->DrawString(L"V-Channel (Brightness)", -1, &font, PointF(10, height + 10), &yellowBrush);
		else;
	...
	else if (st_mon2.disp_mode == AUXPOL_CODE_MON2_DISP_APRM_DIFF) {//自動パラメータセットON/OFF差分画像の表示

		if (!diff_mat.empty()) {

			std::lock_guard<std::mutex> lock(diffmatMutex);//diff_mat,m_allContoursをロック

			// 1. 表示用に 3ch に変換
			cv::Mat RGBmat;
			cv::cvtColor(diff_mat, RGBmat, cv::COLOR_GRAY2BGR);

			// 2. すべての輪郭を描画
			// 全輪郭を水色(0, 255, 0)で、太さ1で描画
			// 引数: (対象画像, 輪郭リスト, 全描画指定(-1), 色, 太さ)
			cv::drawContours(RGBmat, m_allContours, -1, cv::Scalar(255, 255, 0), 2);
			// (オプション) 評価時のROI枠も描画しておく
			cv::rectangle(RGBmat, boundingBox, cv::Scalar(255, 255, 255), 2); // 白枠

			// (オプション) 現在のROI枠も描画しておくと分かりやすいです
			cv::rectangle(RGBmat, init_work_roi, cv::Scalar(0, 255, 255), 2); // ﾏｾﾞﾝﾀ枠

			// 3. GDI+ Bitmap作成
			Gdiplus::Bitmap bitmap(RGBmat.cols, RGBmat.rows, (int)RGBmat.step, PixelFormat24bppRGB, RGBmat.data);

			// 4. 描画
			width = RGBmat.cols / AUXPOL_CAMERA_DISP_RETIO;
			height = RGBmat.rows / AUXPOL_CAMERA_DISP_RETIO;
			m_pOffscreenGraphics->DrawImage(&bitmap, 0, height, width, height);

			// テキスト表示
			Font font(L"Arial", 9, FontStyleBold);
			SolidBrush yellowBrush(Color::Yellow);
			m_pOffscreenGraphics->DrawString(L"ON/OFF Diff with Contours", -1, &font, PointF(10, height + 10), &yellowBrush);
			m_pOffscreenGraphics->DrawString(st_aux_pol_inf.st_img_proc.video_delay_auto_prm_status.c_str(), -1, &font, PointF(10, height + 25), &yellowBrush);
		}
		//ROI描画
		if (st_mon2.flg_disp_cri_roi) {
			m_pOffscreenGraphics->DrawRectangle(m_pGreenPen.get(), rc_mat_roi_criterion_disp.x, rc_mat_roi_criterion_disp.y + height, rc_mat_roi_criterion_disp.width, rc_mat_roi_criterion_disp.height);
...
void COteAuxPol::HSV_autoCalibrate(int type) {
	...
	//ROIが画像サイズを超えないように補正
	cv::Rect valid = pImgPrm->rc_mat_roi_criterion & cv::Rect(0, 0, width, height);
...
void COteAuxPol::UpdateSliderPos() {
	SendMessage(st_mon2.hctrl[AUXPOL_ID_MON2_SLIDER_HL], TBM_SETPOS, TRUE, pst_img_proc->param.hl);
	SendMessage(st_mon2.hctrl[AUXPOL_ID_MON2_SLIDER_HH], TBM_SETPOS, TRUE, pst_img_proc->param.hh);
	SendMessage(st_mon2.hctrl[AUXPOL_ID_MON2_SLIDER_SL], TBM_SETPOS, TRUE, pst_img_proc->param.sl);
	SendMessage(st_mon2.hctrl[AUXPOL_ID_MON2_SLIDER_SH], TBM_SETPOS, TRUE, pst_img_proc->param.sh);
	SendMessage(st_mon2.hctrl[AUXPOL_ID_MON2_SLIDER_VL], TBM_SETPOS, TRUE, pst_img_proc->param.vl);
	SendMessage(st_mon2.hctrl[AUXPOL_ID_MON2_SLIDER_VH], TBM_SETPOS, TRUE, pst_img_proc->param.vh);
...
cv::Rect COteAuxPol::get_hsv_criterion(int mode){
	...
	if (mode == AUXPOL_MODE_AUTO_CAL_AUTO) {
		...
		if (st_mon2.flg_dispDragging) {
			...
			//画像内にあれば処理後の範囲をモニタ画面スケールの選択範囲として採用
...
cv::Rect COteAuxPol::set_work_roi(int mode) {
	...
	//画像内にない時は作業エリア=基準判定エリア
	if (rc_disp.width <= 0 || rc_disp.height <= 0)
		rc_disp = rc_mat_roi_criterion_disp;
	
	//モニタスケール→カメラ画像スケールに変換してセット
	rc_mat_roi_work = { 
		rc_disp.x * AUXPOL_CAMERA_DISP_RETIO,
		rc_disp.y * AUXPOL_CAMERA_DISP_RETIO,
		rc_disp.width * AUXPOL_CAMERA_DISP_RETIO, 
	...
	//Size 0はエラーになるので、最小1に補正
	if (rc_mat_roi_work.width == 0)rc_mat_roi_work.width = 1;
	if (rc_mat_roi_work.height == 0)rc_mat_roi_work.height = 1;
