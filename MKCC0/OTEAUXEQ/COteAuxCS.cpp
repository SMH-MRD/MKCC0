...
ST_CS_MON1 COteAuxCS::st_mon1;
ST_CS_MON2 COteAuxCS::st_mon2;
...
HRESULT COteAuxCS::initialize(LPVOID lpParam) {
	HRESULT hr = S_OK;
	//### 出力用共有メモリ取得
	out_size = sizeof(ST_AUX_CS_INF);
	if (OK_SHMEM != pCsInfObj->create_smem(SMEM_OTE_AUX_CS_INF_NAME, sizeof(ST_AUX_CS_INF), MUTEX_OTE_AUX_CS_INF_NAME)) {
		return(FALSE);
	}
	set_outbuf(pCsInfObj->get_pMap());

	//### 入力用共有メモリ取得
	if (OK_SHMEM != pEnvInfObj->create_smem(SMEM_OTE_AUX_ENV_INF_NAME, sizeof(ST_AUX_ENV_INF), MUTEX_OTE_AUX_ENV_INF_NAME)) {
		return(FALSE);
	}
	if (OK_SHMEM != pAgentInfObj->create_smem(SMEM_OTE_AUX_AGENT_INF_NAME, sizeof(ST_AUX_AGENT_INF), MUTEX_OTE_AUX_AGENT_INF_NAME)) {
		return(FALSE);
	}
...
HRESULT COteAuxCS::routine_work(void* pObj) {
	input();
	parse();
	output();
	return S_OK;
...
LRESULT CALLBACK COteAuxCS::Mon1Proc(HWND hWnd, UINT msg, WPARAM wp, LPARAM lp) {
	switch (msg)
	{
	case WM_CREATE: {
...
LRESULT CALLBACK COteAuxCS::Mon2Proc(HWND hWnd, UINT msg, WPARAM wp, LPARAM lp) {
	switch (msg)
	{
	case WM_CREATE: {
		InitCommonControls();//コモンコントロール初期化
...
HWND COteAuxCS::open_monitor_wnd(HWND h_parent_wnd, int id) {
...
void COteAuxCS::close_monitor_wnd(int id) {
	wos.str(L"");
	if (id == BC_ID_MON1) {
...
void COteAuxCS::show_monitor_wnd(int id) {
...
void COteAuxCS::hide_monitor_wnd(int id) {
	if ((id == BC_ID_MON1) && (st_mon1.hwnd_mon != NULL)) {
...
/****************************************************************************/
LRESULT CALLBACK COteAuxCS::PanelProc(HWND hDlg, UINT msg, WPARAM wp, LPARAM lp) {
...
void COteAuxCS::msg2listview(wstring wstr) {
...
void COteAuxCS::set_PNLparam_value(float p1, float p2, float p3, float p4, float p5, float p6) {
	wstring wstr;
...
void COteAuxCS::set_panel_tip_txt() {
...
void COteAuxCS::set_func_pb_txt() {
...
void COteAuxCS::set_item_chk_txt() {
	wstring wstr_type; wstring wstr;
	switch (inf.panel_func_id) {
	case IDC_TASK_FUNC_RADIO1: {
